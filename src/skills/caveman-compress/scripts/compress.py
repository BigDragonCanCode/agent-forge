...
def call_codex(prompt: str) -> str:
    ...
    with tempfile.NamedTemporaryFile(mode="r+", suffix=".txt") as output_file:
        try:
            subprocess.run(
                ["codex", "exec", "--skip-git-repo-check", "--ephemeral", "-o", output_file.name, "-"],
                input=prompt,
                text=True,
                capture_output=True,
                check=True,
...
def call_openai(prompt: str) -> str:
    ...
    return strip_llm_wrapper(response.output_text.strip())
